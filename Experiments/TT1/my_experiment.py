...
class Experiment:
    ...
    def run(self):
        ...
        run_start = self.ttl_clock.get_time()
        # Loop over each trial
        for ind, row in self.run_info.iterrows():
            self.trial_num = ind+1
            start_time  = run_start +row['start_time']
    ...
    def run_trial(self,row):
        ...
        if 'go_signal' in row.index:
            ...
            # change stim color
            self.ttl_clock.wait_until(start_time_real + go_time)
        ...
        # Show fixation cross
        self.screen.fixation_cross()
        self.ttl_clock.wait_until(start_time_real + go_time + resp_time + iti_dur)
