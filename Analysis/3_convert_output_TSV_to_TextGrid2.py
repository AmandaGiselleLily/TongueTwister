...
def save_onset_offset_textgrid(
    ...
    offset_times : array-like
        Syllable offset times in seconds.

    output_path : str
        Path where TextGrid will be saved.
    ...
    for i in range(len(onset_times)):
        ...
        if (
            ...
            and onset_times[i] - wav_duration
            <= TIME_TOLERANCE_SEC):
            onset_times[i] = wav_duration

        if (
            offset_times[i] > wav_duration
            and offset_times[i] - wav_duration
            <= TIME_TOLERANCE_SEC):
            offset_times[i] = wav_duration
...
def output_tsv_to_textgrids(
    ...
    df["onset_sec"] = pd.to_numeric(
        df["onset_sec"],
        errors="raise",
    )

    df["offset_sec"] = pd.to_numeric(
        df["offset_sec"],
        errors="raise",
    )
    ...
    for wav_filename, trial_df in grouped:
        ...
        try:
            ...
            #
            # copy=True is important because these arrays will later be
            # modified when correcting floating-point boundary differences.
            # -----------------------------------------------------------------
